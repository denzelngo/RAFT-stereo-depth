...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if np.random.rand() < 0.5:
            ...
            img1 = kornia.filters.motion_blur(img1.unsqueeze(0), 15, angle, 0)
            img2 = kornia.filters.motion_blur(img2.unsqueeze(0), 15, angle, 0)
...
class SceneFlowDatasets(StereoDataset):
    ...
    def _add_things(self, split='TRAIN'):
        ...
        root = osp.join(self.root, self.dstype, 'FlyingThings3D')
        if split == 'TRAIN':
            left_images = sorted(glob(osp.join(root, split, '*/*/left/*.png'))[::self.sample_rate])
        else:
            left_images = sorted(glob(osp.join(root, split, '*/*/left/*.png')))
    ...
    def _add_monkaa(self):
        ...
        original_length = len(self.disparity_list)
        root = osp.join(self.root, self.dstype, 'Monkaa')
        left_images = sorted(glob(osp.join(root, '*/left/*.png'))[::self.sample_rate])
    ...
    def _add_driving(self):
        ...
        original_length = len(self.disparity_list)
        root = osp.join(self.root, self.dstype, 'Driving')
        left_images = sorted(glob(osp.join(root, '*/*/*/left/*.png'))[::self.sample_rate])
        right_images = [image_file.replace('left', 'right') for image_file in left_images]
        disparity_images = [im.replace(self.dstype, 'disparity').replace('.png', '.pfm') for im in left_images]
...
class TartanAir(StereoDataset):
    def __init__(self, aug_params=None, root='datasets', keywords=[]):
        ...
        assert os.path.exists(root)
        filenames = glob(osp.join(root, '*/Easy/*/image_left/*.png'))[::15]
...
class KITTI(StereoDataset):
    def __init__(self, split='KITTI2015', aug_params=None, root='datasets/KITTI', image_set='training'):
...
class UASOL(StereoDataset):
    def __init__(self, root='datasets', training=True):
        super().__init__(reader=frame_utils.readDispUASOL)

        image1_list = glob(osp.join(root, 'images/*left*.png'))
        image2_list = [x.replace('left', 'right') for x in image1_list]
        disp_list = [x.replace('images', 'depth').replace('left', '').replace('color', 'depth') for x in image1_list]
...
def fetch_dataloader(args, root):
    ...
    for dataset_name in args.train_datasets:
        if dataset_name.startswith("middlebury_"):
            ...
            final_dataset = SceneFlowDatasets(root=osp.join(root, 'SceneFlow'), dstype='frames_finalpass')
            ...
            new_dataset = TartanAir(root=osp.join(root, 'Tartanair', 'DATA'))
...
def fetch_validation_dataloader(args, root):
    val_loaders = []
    batch_size = args.batch_size
    for dataset_name in args.valid_datasets:
        if dataset_name.startswith("middlebury_"):
            new_dataset = Middlebury(split=dataset_name.replace('middlebury_', ''))
        elif dataset_name == 'sceneflow':
            # clean_dataset = SceneFlowDatasets(aug_params, dstype='frames_cleanpass')
            new_dataset = SceneFlowDatasets(root=osp.join(root, 'SceneFlow'), dstype='frames_finalpass',
                                            things_test=True)
            print(f"Adding {len(new_dataset)} samples from SceneFlow")
        elif 'kitti' in dataset_name:
            new_dataset = KITTI(split=dataset_name)
            print(f"Adding {len(new_dataset)} samples from KITTI")
        elif dataset_name == 'sintel_stereo':
            new_dataset = SintelStereo()
            print(f"Adding {len(new_dataset)} samples from Sintel Stereo")
        elif dataset_name == 'falling_things':
            new_dataset = FallingThings()
            print(f"Adding {len(new_dataset)} samples from FallingThings")
        elif dataset_name.startswith('tartan_air'):
            new_dataset = TartanAir(root=osp.join(root, 'Tartanair', 'DATA'))
            print(f"Adding {len(new_dataset)} samples from Tartain Air")
        elif dataset_name == 'eth3d':
            new_dataset = ETH3D(root=osp.join(root, 'ETH3D'), training=False)
            print(f"Adding {len(new_dataset)} samples from ETH3D")
        elif dataset_name == 'uasol':
            new_dataset = UASOL(root=osp.join(root, 'UASOL'), training=False)
            print(f"Adding {len(new_dataset)} samples from UASOL")
        else:
            raise Exception(f'{dataset_name} not implemented yet!!')
        val_loaders.append(data.DataLoader(new_dataset, batch_size=batch_size,
